/* Agentic Decision Mathematics — client.
...
(() => {
  ...
  const num = (v, d = 2) => (v == null ? "—" : Number(v).toLocaleString("en-GB", { minimumFractionDigits: d, maximumFractionDigits: d }));
  const eur = (v) => (v == null ? "—" : `€${Math.round(v).toLocaleString("en-GB")}`);
  const signed = (v, d = 0) => `${v >= 0 ? "+" : "−"}${Math.abs(v * 100).toFixed(d)}%`;
  ...
  async function load(target, fn) {
    target.innerHTML = `<div class="loading">Computing…</div>`;
    try {
    ...
    return `
      <div class="stack">
        <div class="sheet">
          ...
          <div class="facts"><span>Notional <b>€${(c.notional / 1e6).toFixed(1)}m</b></span><span>Cutoff in <b>${num(c.cutoff_h, 1)} h</b></span>
        ...
        <div class="grid g2">
          ...
          <div class="sheet"><h3>Belief state</h3>
            ${hyp.map((h, i) => `<div class="belief-row" data-tip="${esc(h.label)}<br>prior ${pct(d.prior[i])} → posterior ${pct(d.belief[i])}">
        ...
        <div class="agents">${d.agents.map((a) => `
          <div class="agent"><h3>${esc(a.name)}</h3><div class="small faint">${esc(a.style)}</div>
            <dl>
              ...
              <dt>Conformal set</dt><dd>${a.conformal_set.map((x) => `<span class="tag">${esc(x)}</span>`).join("")} <span class="faint small">α=${a.alpha}</span></dd>
              ...
              <dt>Binding</dt><dd class="small">${a.binding.length ? a.binding.map((b) => `<span class="tag">${esc(b)}</span>`).join("") : "—"}</dd>
          ...
          <div class="verdict">
            ...
            <div>
              ...
              <p class="small muted" style="margin:0 0 8px">Expected: <b>${d.by_mode.expected}</b> · Tail-aware: <b>${d.by_mode.cvar}</b> · Robust: <b>${d.by_mode.robust}</b>.
                Calibration for this class at α=${num(d.calibration.alpha, 4)}: ${esc(d.calibration.reason)}. Drift: ${d.drift.stable ? "stable" : `material (W₁ ${d.drift.w1}, KL ${d.drift.kl}${d.drift.cusum_alarm ? ", CUSUM alarm" : ""})`}.</p>
              ${eq(modeTex)}
        ...
        <div class="sheet"><h3>Every candidate action</h3>
          <div class="table-wrap"><table><thead><tr><th>Action</th><th>Feasibility</th><th class="num">E[L]</th><th class="num">CVaR₉₅</th><th class="num">Risk R</th><th class="num">P(fail)</th><th class="num">E[T] h</th><th class="num">Human min</th><th>${d.mode === "robust" ? "Worst-case J" : "Objective J"}</th><th>Autonomy</th><th>Binding caps</th></tr></thead><tbody>
            ${d.actions.map((a) => `<tr class="${a.optimal ? "best" : a.feasible ? "" : "off"}">
              ...
              <td>${a.feasible ? `<div style="display:flex;gap:8px;align-items:center"><div class="bar" style="flex:1"><i style="width:${(jOf(a) / maxJ) * 100}%"></i></div><span class="num" data-tip="${Object.entries(a.weighted).map(([k, v]) => `${k}: ${v}`).join("<br>")}">${num(jOf(a), 3)}</span></div>` : "—"}</td>
    ...
    return `
      <div class="stack">
        ...
        <div class="sheet"><div class="table-wrap"><table><thead><tr><th>Agent</th><th class="num">Valid</th><th class="num">Control-safe</th><th class="num">Matches optimum</th><th class="num">Mean regret</th><th class="num">Human min/case</th><th class="num">Resolution</th><th class="num">Tail loss CVaR₉₅</th><th class="num">Autonomous</th></tr></thead><tbody>
          ...
          <tr class="best"><td>${esc(r.reference.label)}</td><td class="num">100%</td><td class="num">—</td><td class="num">100%</td><td class="num">0.000</td><td class="num">—</td><td class="num">${pct(r.reference.resolution)}</td><td class="num">${eur(r.reference.tail_loss_cvar95)}</td><td class="num">—</td></tr>
          </tbody></table></div>
  ...
  function scatter(points, th) {
    ...
    bands.forEach(([a, b, l]) => {
      ...
      s += `<text x="${w - m.r - 6}" y="${Y(b) + 14}" text-anchor="end" font-size="11" fill="${LEVEL_COLOR[l]}" font-weight="600">${l === "DENY" ? "DENY" : `≤ ${l}`}</text>`;
    });
    ...
    return `
      <div class="stack">
        ...
        <div class="sheet"><div class="section-head"><h2>Calibration Registry</h2><span class="small muted">${esc(d.agent)} · split conformal per exception class</span></div>
          <div class="table-wrap"><table><thead><tr><th>Exception class</th><th class="num">History</th><th class="num">n cal</th><th class="num">q̂ at α=0.05</th><th class="num">Coverage</th><th class="num">Wilson LB</th><th class="num">E|Γ|</th><th>Status</th><th>By action (burden of proof)</th></tr></thead><tbody>
            ${d.registry.map((r) => `<tr><td>${esc(r.name)}</td><td class="num">${r.history.toLocaleString("en-GB")}</td><td class="num">${r.headline.n_cal.toLocaleString("en-GB")}</td>
              <td class="num">${r.headline.q_hat == null ? "∞" : num(r.headline.q_hat, 3)}</td><td class="num">${pct(r.headline.coverage)}</td><td class="num">${pct(r.headline.coverage_lb)}</td><td class="num">${num(r.headline.mean_set_size, 2)}</td>
              <td>${r.headline.calibrated ? `<span class="lv lv-AUTO">CALIBRATED</span>` : `<span class="tag warn">${esc(r.headline.reason)}</span>`}</td>
              <td><div style="display:flex;gap:4px">${r.by_action.map((a) => `<span data-tip="${esc(a.action)}<br>α = ${a.alpha}<br>coverage LB ${pct(a.coverage_lb)}<br>set size ${a.set_size}<br>${a.calibrated ? "calibrated" : "not calibrated"}" style="width:14px;height:14px;border-radius:50%;display:inline-block;${a.calibrated ? "background:#0e7c66" : "border:1.5px solid #b42318"}"></span>`).join("")}</div></td></tr>`).join("")}
          </tbody></table></div>
        ...
        <div class="sheet"><div class="section-head"><h2>Drift monitor</h2><span class="small muted">Baseline days 1–${d.drift_thresholds.baseline_days}; shaded live window to day ${d.drift_thresholds.days}</span></div>
          <div class="table-wrap"><table><thead><tr><th>Exception class</th><th class="num">W₁ scores</th><th class="num">KL mix</th><th>Daily error rate</th><th>CUSUM</th><th>Status</th></tr></thead><tbody>
  ...
  function viewSensitivity() {
    ...
    return `
      <header class="screen-head"><h1>Policy Sensitivity</h1>
        <p>The weights λ are governance objects, not code constants. This screen shows exactly where the recommendation changes, and whether a disagreement is about the facts or about risk appetite.</p></header>
      <div class="sheet stack">
        <div class="controls">
  ...
  function sensHtml(d) {
    ...
    const sweepRows = Object.entries(d.sweeps).map(([k, sw]) => {
      ...
      const span = 5;
      const bar = segs.map((s) => `<i style="width:${((s.end - s.start) / span) * 100}%;background:${ACTION_COLOR[s.action]}" data-tip="${esc(s.action)}: ${s.start} – ${s.end}">${(s.end - s.start) / span > 0.14 ? esc(s.action) : ""}</i>`).join("");
      let text;
      if (segs.length === 1) text = `<b>${esc(segs[0].action)}</b> stays optimal for every ${tex(LAMBDA_TEX[k])} in [0, 5].`;
      else text = segs.slice(1).map((s, i) => `${esc(segs[i].action)} → <b>${esc(s.action)}</b> when ${tex(LAMBDA_TEX[k])} ${sw.current < s.start ? "exceeds" : "passes"} <b>${num(s.start, 2)}</b>`).join("; ");
    ...
    return `
      <div class="grid g2">
        <div class="sheet"><h3>Phase diagram · ${esc(d.case.title)}</h3>
          <div class="phase-wrap"><div class="ylab">${esc(ly)} ${tex(LAMBDA_TEX[d.y_key])} → ${d.ys[res - 1]}</div>
            <div class="phase" style="grid-template-columns:repeat(${res},1fr)">${cells}</div>
            <div class="xlab">${esc(lx)} ${tex(LAMBDA_TEX[d.x_key])} → ${d.xs[res - 1]}</div></div>
    ...
    return `
      <div class="stack">
        ...
        <div class="kpis">
          ...
          <div class="kpi"><div class="v">${pct(s.fte_reduction_pct)}</div><div class="k">FTE reduction · ${d.baseline.fte} → ${s.fte}</div></div>
        ...
        <div class="grid g2">
          ...
          <div class="sheet"><h3>Why minutes removed ≠ people released</h3>
            <table><tbody>
              <tr><td>Minutes removed by the allocated autonomy</td><td class="num"><b>${pct(s.minutes_removed_pct)}</b></td></tr>
              <tr><td>Lost to fragmentation: partial automation frees slivers of tasks, not whole tasks</td><td class="num">−${pct(s.decomposition.fragmentation)}</td></tr>
              <tr><td>Lost to staffing rigidity: integer FTE, stress buffer, control floors (${s.floor_binding_cells} floor-bound cells)</td><td class="num">${s.decomposition.staffing_rigidity >= 0 ? "−" : "+"}${pct(Math.abs(s.decomposition.staffing_rigidity))}</td></tr>
        ...
        <div class="sheet"><h3>Staffing by skill and region · baseline → optimised</h3>
          <div class="table-wrap"><table><thead><tr><th>Skill</th>${regions.map((r) => `<th class="num">${r}</th>`).join("")}<th class="num">Floor</th></tr></thead><tbody>
            ${skills.map((sk) => `<tr><td>${esc(sk)}</td>${regions.map((rg) => { const c = cell(sk, rg); return `<td class="num">${c.baseline} → <b>${c.optimised}</b>${c.optimised === c.floor ? ` <span class="tag">floor</span>` : ""}</td>`; }).join("")}<td class="num">${cell(sk, regions[0]).floor}</td></tr>`).join("")}
            <tr><td>Flexible pool</td>${s.flex.regions.map((_, i) => `<td class="num">${s.flex.baseline[i]} → <b>${s.flex.optimised[i]}</b></td>`).join("")}<td></td></tr>
          </tbody></table></div>
      ...
      <div class="stack">
        ...
        <div class="grid g2">
          ...
          <div class="sheet"><h3>Tamper-evident decision log</h3>
            ...
            <p class="small" style="margin:12px 0 4px">Tamper test: event ${d.chain.tamper_test.modified_seq} altered to claim AUTO → ${d.chain.tamper_test.verified ? "not detected" : `<b>detected at event ${d.chain.tamper_test.first_break}</b>`}</p>
            <table class="log"><tbody>${d.chain.tail.map((c) => `<tr><td class="num">${c.seq}</td><td class="hash">${esc(c.hash.slice(0, 32))}…</td></tr>`).join("")}</tbody></table>
          </div>
        </div>
  ...
  async function boot() {
    const main = $("#main");
    main.innerHTML = `<div class="loading">Loading catalogue…</div>`;
    try {
